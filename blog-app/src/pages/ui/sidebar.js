...
import { IconMenu2, IconArrowLeft, IconBrandTabler, IconSettings, IconUserBolt, IconHelpCircle } from "@tabler/icons-react";
...
export function SidebarDemo() {
  ...
  return (
    <div className="flex">
      {/* Sidebar rétractable sur mobile, toujours visible sur grand écran */}
      <div
        className={`fixed top-0 left-0 h-full bg-gray-900 text-white shadow-xl z-50 lg:relative lg:w-64 transition-transform duration-300 ${
            ...
            <div className="mt-auto flex flex-col gap-2 border-t border-gray-700 pt-4">
              <SidebarLink
                link={{
                  label: "Aide",
                  href: "/help",
                  icon: <IconHelpCircle className="text-gray-400 h-6 w-6" />,
                }}
                open={open}
              />
              <SidebarLink
      ...
      {/* Bouton d'ouverture de la sidebar (affiché uniquement sur mobile) */}
      <div className="p-4 lg:hidden">
        <button
          onClick={() => setOpen(!open)}
          className="text-white bg-gray-800 p-2 rounded-lg shadow-lg hover:bg-cyan-600 transition-all duration-300"
        >
...
export const Logo = ({ open, setOpen }) => (
  <button
    onClick={() => setOpen(!open)}
    className="flex items-center space-x-2 text-sm text-white py-6 border-b border-gray-700 w-full justify-center"
  >
...
export function SidebarLink({ link }) {
  return (
    <Link
      href={link.href}
      className="sidebar-link flex items-center space-x-3 p-3 hover:bg-cyan-600 rounded-lg transition-all duration-300 hover:text-white"
    >
      {link.icon}
